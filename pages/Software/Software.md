---

title: "Software"  
linkTitle: "Software"  
weight: 40  
description: Software resources and development topics
type: [[Software]]
---

## Overview

This section covers software development, tools, platforms, and technologies across various domains including web development, machine learning, DevOps, and more.

## Sub-Categories

### Development & Engineering
- [[Programming]]
- [[Development]]
- [[Software Engineering]]
- [[Development Environments]]
- [[Word Processing]]

### Cloud & Infrastructure
- [[Cloud]]
- [[DevOps]]

### Data & AI
- [[Data]]
- [[MachineLearning]]
- [[Natural Language Processing]]

### Web & Applications
- [[WebDevelopment]]
- [[Applications]]
- [[Games]]

### Specialized Topics
- [[Blockchain]]
- [[Graphic Design]]
- [[Simulation]]
- [[NVIDIA]]
- [[Open metaverse]]
- [[OpenSource]]

## Related Topics
- [[Enterprise]]
- [[Freelancing]]
- [[Linux]]
- [[Packages]]
- [[Robotic Process Automation]]
- [[ssh]]

## Technology Stacks

### Python Development
- **Package Management:** Poetry
- **CLI Development:** Typer
  - Example: password generator
- **REST API:** FastAPI
  - Example: password generator
- **Cloud Computing:**
  - Function as a Service (FaaS)
    - OpenFaaS
    - AWS Lambda
    - Azure Functions
    - Google Cloud Functions
- **Databases:**
  - SQL: Neon (Serverless Postgres)
  - NoSQL: MongoDB

### Machine Learning Applications
- Digital advertising
- Social network feeds
- Recommendation systems
- Computer Vision
  - Object detection
  - Semantic segmentation
- Natural Language Processing
  - Text to speech
- Search ranking
- MLOps

### Robotics & Autonomous Systems
- Mobile robots
- Quadrotors
- ROS2
- Isaac Sim
- **Autonomous Driving:**
  - CARLA
  - Autoware

### Web Development
- **Frontend Frameworks:**
  - ReactJS
  - VueJS
  - AngularJS
- **Backend Development**

### IoT & Embedded Systems
- **IoT Platforms:**
  - [Home Assistant](https://www.home-assistant.io/)
- **Embedded Hardware:**
  - STM32
  - Arduino
  - Teensy
  - ESP32

## Platform-Specific Resources

### MacOS
- [Mac App Store](http://macappstore.org/)
- [USB Overdrive](http://www.usboverdrive.com/)
- [Install nvm](https://nodesource.com/blog/installing-node-js-tutorial-using-nvm-on-mac-os-x-and-ubuntu/)
- [Change the default terminal to bash](https://www.howtogeek.com/444596/how-to-change-the-default-shell-to-bash-in-macos-catalina/)

### Windows
- Remove any external hard drive or flash memory before installation
- [Install Windows 10 on Asus ROG](https://rog.asus.com/forum/showthread.php?76993-Guide-How-to-install-windows-8-8-1-10-The-proper-way-(UEFI-based-installation))
- [Nano text editor for Windows](https://files.lhmouse.com/nano-win/)
- [Fix the Windows 10 time in dual boot](https://technastic.com/fix-time-between-windows-10-and-ubuntu-in-dual-boot/)

### WSL (Windows Subsystem for Linux)
- [Convert WSL1 to WSL2](https://ericsysmin.com/2019/07/13/converting-wsl-1-operating-systems-to-wsl-2-on-windows/)

### PowerShell
- `Get-Command ssh` - Check SSH availability
- [Nubotics](https://www.nubotics.io/)
